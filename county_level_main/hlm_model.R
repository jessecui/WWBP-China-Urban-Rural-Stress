...
df$IsUrban = as.integer(df$CountyTierLevel <= 2)

# Takes Log CountyGDPPerCapita
df$CountyGDPPerCapita2012Yuan <- log(df$CountyGDPPerCapita2012Yuan)
...
outcomes <- c('IsUrban', 'CountyGDPPerCapita2012Yuan')
controls <- c('None')
...
for (i in unique_feats) {
  ...
  # Urban + County GDP HLM
  m <-
    lmer(
      group_norm ~ gender_isFemale 
      + (1 |city_id) 
      + IsUrban 
      + CountyGDPPerCapita2012Yuan,
      data = table
    )
  model_data <- c()
